...
from app.models import TableauDashboardGrade, PAIRReportsGrade, TableauDashboardV2Grade
from app.api.errors import error_response, bad_request
from flask import jsonify, g
from sqlalchemy.orm.exc import NoResultFound

#
# Routes for Tableau Dashboard v2
#
@bp.route('/v3/grades/<string:campus>/<yearsession:ys>', methods=['GET'])
def get_grades_v3_l1(campus, ys):
    result = [row.to_dict() for row in TableauDashboardV2Grade.query.filter_by(
        campus=campus, year=ys.year, session=ys.session).all()]
    return jsonify(result) if result != [] else error_response(404, "Not Found")


@bp.route('/v3/grades/<string:campus>/<yearsession:ys>/<string:subject>', methods=['GET'])
def get_grades_v3_l2(campus, ys, subject):
    result = [row.to_dict() for row in TableauDashboardV2Grade.query.filter_by(
        campus=campus, year=ys.year, session=ys.session, subject=subject).all()]
    return jsonify(result) if result != [] else error_response(404, "Not Found")


@bp.route('/v3/grades/<string:campus>/<yearsession:ys>/<string:subject>/<string:course>', methods=['GET'])
def get_grades_v3_l3(campus, ys, subject, course):
    result = [row.to_dict() for row in TableauDashboardV2Grade.query.filter_by(
        campus=campus, year=ys.year, session=ys.session, subject=subject, course=g.course, detail=g.detail).all()]
    return jsonify(result) if result != [] else error_response(404, "Not Found")


@bp.route('/v3/grades/<string:campus>/<yearsession:ys>/<string:subject>/<string:course>/<string:section>', methods=['GET'])
def get_grades_v3_yearsession_subject_course_section(campus, ys, subject, course, section):
    try:
        result = TableauDashboardV2Grade.query.filter_by(campus=campus, year=ys.year, session=ys.session, subject=subject,
